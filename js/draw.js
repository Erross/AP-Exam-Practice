...
(function (root, factory) {
  ...
  /**
   * Reorder a fully-drawn exam into named, contiguous parts — e.g. a
   * no-calculator Part A followed by a calculator-required Part B — the way a
   * real multi-part MCQ section is actually delivered and timed section by
   * section, not just balanced in aggregate.
   *
   * `subject.examParts` is `{ field: "calculatorAllowed", parts: [{ value:
   * false, label, timeMinutes }, { value: true, label, timeMinutes }] }`.
   *
   * Every stimulus set must be homogeneous in `field` — a set that straddles
   * two parts cannot be delivered as one contiguous block in either part, so
   * that is a hard content error surfaced here, not something this function
   * silently works around. Blocks keep whatever relative order the draw
   * already produced; this only partitions them by part, preserving that
   * order within each partition, so the earlier random shuffle still governs
   * the order questions appear within Part A and within Part B.
   */
  ...
  function drawExam(subject, bank, rng) {
    ...
    if (units.length === 0) {
      ...
      if (subject.examBlueprint) {
        ...
        for (let attempt = 0; attempt < attempts; attempt++) {
          ...
          // If a weighted pool cannot cover the configured draw, try another
          // whole-block combination before failing. Quietly borrowing from
          // another unit would make the delivered blueprint untrue.
    ...
    // Multi-part MCQ sections (e.g. a no-calculator Part A followed by a
    // calculator-required Part B) need their delivery order regrouped by
    // part, on top of whatever constraint-satisfying draw produced `result`.
