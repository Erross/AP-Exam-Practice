...
const ranges=s.skillCountRanges || (s.attributeRanges && s.attributeRanges.skill);
if(!ranges) throw new Error('No skill ranges configured');
const targets=apportion(s.units.map(u=>({id:u.id,weight:u.examWeight,capacity:bank.filter(q=>q.unit===u.id).length})),42);
console.log('TARGETS',targets);console.log('RANGES',ranges);
...
const max=skills.map(k=>ranges[k][1]),min=skills.map(k=>ranges[k][0]);
...
for(let mask=0;mask<(1<<groups.length);mask++){const pc=mask.toString(2).replace(/0/g,'').length;if(pc<2||pc>4)continue;let states=[[0,0,0,0,0,0]],ok=true;
 ...
 if(ok){const hit=states.find(v=>v.every((x,i)=>x>=min[i]&&x<=max[i]));if(hit)feasible.push({sets:groups.filter((_,i)=>mask&(1<<i)),skills:Object.fromEntries(skills.map((k,i)=>[k,hit[i]]))})}}
