...
// contains five complete passages: two 12-question short-fiction sets, two
// 11-question poetry sets, and one 9-question longer-fiction/drama set.
...
(function () {
  ...
  const QUALIFIERS = {
    2: ["in context", "as presented", "as described", "as written"],
    3: ["in the passage", "in the text", "as presented here", "within this work"],
    4: ["in the passage overall", "across the text overall", "within the work's argument", "as the passage develops"],
    5: ["as the passage presents it", "within the text's full argument", "as this passage develops it", "in the work as presented"],
    6: ["within the passage as a whole", "in the full context presented here", "as the text develops the idea"],
    7: ["in the full context of this passage", "within the broader reasoning of the passage"],
    8: ["within the broader context established by this passage"],
    9: ["in the broader context established by this passage overall"],
  };
  const words = (text) => text.trim().split(/\s+/).length;
  function balanceOptions(options, correctIndex, questionIndex) {
    const lengths = options.map(words);
    const max = Math.max(...lengths);
    const pads = lengths.map((length) => {
      const amount = Math.min(3, max - length);
      return amount === 1 ? 0 : amount;
    });
    if (questionIndex % 2 === 0) {
      const distractors = lengths.map((length, index) => index === correctIndex ? -1 : length + pads[index]);
      const index = distractors.indexOf(Math.max(...distractors));
      const need = Math.max(...lengths.map((length, i) => length + pads[i])) - distractors[index] + 1;
      if (pads[index] + need <= 9) pads[index] += need;
    }
    pads.forEach((amount, index) => { if (amount === 1) pads[index] = 2; });
    return options.map((option, index) => {
      if (!pads[index]) return option;
      const punctuation = /[.!?]$/.test(option) ? option.slice(-1) : "";
      const base = punctuation ? option.slice(0, -1) : option;
      const choices = QUALIFIERS[pads[index]];
      const qualifier = choices[(questionIndex + index) % choices.length];
      return `${base} ${qualifier}${punctuation}`;
    });
  }
  ...
  function addSet(set, items) {
    ...
    items.forEach((item, index) => {
      ...
      const balancedOptions = balanceOptions(options, correctIndex, QUESTIONS.length);
      QUESTIONS.push({
        ...
        q: item[1], o: balancedOptions, c: [correctIndex], e: item[6],
      });
    });
  }
...
The distinction sounded grander than the object deserved, but she placed the packet in her coat. On the bench, the task lamp cast its perfect circle over a disassembled pocket watch. Mara did not know whether her grandfather had stopped because his hands trembled, because the owner had not returned, or because death had interrupted an ordinary Tuesday. For the first time, she understood that his famous patience had never promised completion. It had promised only that each small thing would be given its turn.`,
    ...
    ["5.B", "The “perfect circle” cast by the task lamp most strongly symbolizes", "a bounded field of attention amid larger uncertainty", "the financial completeness of the shop's records", "Mara's newly acquired mastery of watch repair", "the uncle's desire to preserve every object", "The light isolates one unfinished mechanism and parallels the grandfather's practice of giving each small thing its turn, even though the reasons and outcomes remain uncertain."],
    ["6.B", "The comparison of the labels to the work of “an insect archivist” chiefly conveys", "their extreme minuteness and meticulous organization", "the shop's infestation and physical decay", "Mara's scientific knowledge of insects", "the grandfather's contempt for ordinary handwriting", "The fanciful comparison joins tiny scale with archival care. Nothing indicates insects, contempt, or specialized scientific knowledge."],
    ["7.B", "Which detail best supports an interpretation that the passage values process more than guaranteed results?", "Her grandfather's patience promised that each small thing would receive attention, not that every repair would be finished.", "The buyer wants the display cases but does not want the tools.", "Mara places the packet containing her watch inside her coat.", "Dust has settled throughout most of the shop.", "The final distinction explicitly separates faithful attention from completion, turning the unfinished watches into evidence of a process-centered ethic."],
  ]);
  ...
  addSet({
    ...
    text: `Everyone in Calder said the crossing bell had been silent for years, but I heard it on the morning I left. One clean strike traveled across the river before dawn, the note too deliberate to be ice shifting against the pilings.
...
Perhaps. But when I pass the river now, I lower the car window. Usually there is only tire noise and wind moving under the bridge. Once, in rain, I heard something metallic. I drove on before it could become either a bell or not a bell.`,
    ...
    ["1.D", "The sister's accusation most directly complicates the narrator by suggesting that the narrator", "shapes memories to support a preferred story about leaving", "has forgotten that the sister moved away first", "deliberately damaged the crossing bell", "never actually lived in Calder", "Her claim that the narrator “arranged Calder” casts the earlier certainty as narrative construction, introducing possible unreliability without proving the event false."],
    ["1.E", "By the final sentence, the narrator has changed chiefly by becoming", "less willing to force an ambiguous sound into a definite meaning", "certain that Mr. Vale rang the bell that morning", "indifferent to every memory of Calder", "determined to return and repair the ferry", "Driving on before naming the sound preserves its ambiguity. This differs from the youthful insistence that the note was a deliberate bell struck personally."],
    ["2.A", "The abandoned ferry landing primarily functions as", "a setting where obsolete structures mirror the narrator's attachment to old meanings", "a realistic transportation hub still used by most residents", "a place of danger from which Mr. Vale rescues the narrator", "evidence that Calder's economy depends on river trade", "The weeds, ticket house, absent ferry, and retained bell embody things continuing after their causes have ended, matching the narrator's memory-making."],
    ["3.B", "The revelation that Mr. Vale may already have moved away serves to", "reopen what had seemed a coherent departure memory", "confirm every detail in the narrator's account", "explain why the bus driver was impatient", "resolve the source of the metallic sound in the rain", "The sister's chronology destabilizes the scene and shifts the passage from nostalgic recollection toward an examination of how the narrator constructs it."],
    ["3.D", "The repeated idea that objects remain after their purposes end helps unify", "the town's physical details with the narrator's lingering emotional habits", "the bridge's construction with the ferry's return", "Mr. Vale's clocks with the bus schedule", "the leaking roof with an approaching storm", "Basin, bell, and ferry structures persist, just as the narrator continues listening for a meaning tied to departure long after leaving."],
    ["4.A", "The first-person narration is especially significant because it", "makes the disputed event inseparable from the narrator's need to interpret it", "allows the reader to verify the sister's memory independently", "provides objective access to Mr. Vale's location that winter", "eliminates uncertainty about the final metallic sound", "Every event reaches readers through the person whose emotional need may shape it, so the narrative voice creates rather than solves the central uncertainty."],
    ["4.B", "The statement “There was no boat” chiefly exposes", "the gap between the literal situation and the symbolic departure the narrator desires", "Mr. Vale's successful attempt to deceive the bus driver", "the narrator's ignorance that a bridge has been built", "the sister's plan to follow on a later ferry", "The blunt fact interrupts Mr. Vale's conventional phrase, showing that “boat” and bell belong to a ritual the narrator wants rather than actual transport."],
    ...
    ["5.B", "The basin catching drops from a clear sky most strongly suggests", "a habit or response continuing after the problem that created it", "a supernatural storm confined to the front room", "the sister's refusal to repair the family home", "the narrator's inability to distinguish weather conditions", "The narrator explicitly says the basin remains after the roofer came, making it an image of persistence detached from its original cause."],
    ["6.B", "The statement that the note was “too deliberate to be ice” compares", "meaningful human intention with an impersonal natural sound", "winter travel with warm bus windows", "the bridge's noise with the clocks in Mr. Vale's truck", "the narrator's hearing with the ferryman's eyesight", "The narrator rejects a natural explanation because the sound seems purposeful, revealing the wish to hear an intentional farewell."],
  ...
  addSet({
    id: "sf-room", unit: "SF", type: "short-fiction", era: "pre-20th-century style",
    title: "The West Room (original period-style fiction)", source: "Original fiction created for AP Exam Practice.",
...
The next Thursday he helped the maid beat the counterpane. He did not ask whether the room awaited a man, a body, an apology, or merely the ending of a sentence the family had refused to finish.`,
    ...
    ["1.A", "Arthur's first comment about the room reveals his initial tendency to", "value unused space chiefly by the purpose he can assign it", "understand immediately why his aunt preserves the room", "fear that the room is physically unsafe", "resent his uncle for leaving the family", "Calling the room a fine prospect “given” to nobody makes its use seem obvious to Arthur before he understands that his aunt's ritual gives it another kind of purpose."],
...
Mrs. Niles collected the bowls. At the door she removed the consultant's photograph from the rejected pile and slid it under the leaking window. The women of 1912 disappeared beneath a widening map of rain, still holding their banners above their heads.`,
    ...
    ["1.A", "Chairman Bell is characterized primarily as someone who", "uses procedural language to avoid confronting contradictions in the committee's values", "secretly intends to repair the workers' hall himself", "values labor history more than architectural appearance", "has no awareness that the building is deteriorating", "Bell invokes purpose, reports, criteria, and balance while prioritizing columns over events and meeting inside a condemned candidate. His pauses show awareness without meaningful response."],
    ["1.C", "Mrs. Niles functions as a contrast to Bell chiefly because she", "responds practically to the building and asks directly about lived history", "possesses greater formal authority over the committee vote", "cares only about serving the meal efficiently", "argues that water damage is architecturally distinguished", "She places buckets, questions the criteria, and repurposes the photograph, while Bell abstracts the building into procedure and consultant language."],
    ["1.E", "Mrs. Niles's final action most clearly expresses", "a bitter recognition that the committee's neglect is erasing the history it claims to evaluate", "satisfaction that the photograph will protect the wall permanently", "confidence that the committee will reverse its decision", "indifference toward the women shown in the photograph", "Using the rejected image against the leak literalizes institutional neglect covering the women. The action is pointed and despairing, not a durable repair or sign of confidence."],
    ["2.A", "Holding the meeting in the condemned hall chiefly creates", "situational irony that exposes the committee's abstract approach to preservation", "a neutral setting chosen only because it has a large dining room", "proof that the hall is structurally identical to the mansions", "suspense about whether the founders in the mural will come alive", "The committee debates civic character while dust, leaks, and history surround it, yet members treat the hall as a file rather than the space they occupy."],
    ["3.B", "The committee vote serves as the climax because it", "turns the passage's verbal contradictions into an official act of delay", "unexpectedly guarantees the hall's immediate restoration", "reveals that Mrs. Niles controls the consultant", "resolves the disagreement about architectural criteria", "The vote institutionalizes the preference shown in discussion and directly leads to Mrs. Niles's final symbolic response; it neither repairs nor resolves the underlying issue."],
    ["3.D", "The repeated radiator interruptions function structurally as", "a comic refrain suggesting the ignored building keeps answering the committee", "technical evidence that the boiler is new and efficient", "a signal that Mrs. Niles should serve the next course", "proof that the committee members cannot hear one another", "The personified knocks puncture Bell's statements and make the physical hall seem to object, reinforcing the satire each time procedure overrides present evidence."],
    ["4.A", "The narrator's perspective contributes to the satire by", "describing damage and committee behavior with dry, understated commentary", "entering Bell's thoughts to justify every decision", "explaining directly which buildings deserve preservation", "treating Mrs. Niles's actions as incomprehensible", "Phrases such as dust “seasoning” soup and the radiator “objecting” frame events wryly without delivering a formal policy judgment."],
    ["4.B", "The statement that “criteria required balance” is presented so that readers recognize it as", "an empty abstraction used to shield a preference for conventional grandeur", "a precise mathematical rule the committee consistently applies", "Mrs. Niles's successful defense of the workers' hall", "evidence that historical significance is the committee's highest priority", "Bell never explains the balance; his next point about columns exposes the phrase as cover for privileging architectural prestige over social history."],
...
From the field, the house looked smaller than it did from the road. Eli considered taking the fence down. Instead he straightened the leaning post and left the orange line facing a winter not yet visible.`,
    ...
    ["1.E", "Repairing the fence in September chiefly shows Eli", "assuming a responsibility formerly associated with his father", "accepting that his father will return before winter", "deciding to sell the field and leave the house", "rejecting the practical lesson the fence taught", "Eli acts without the instructions he claimed to await, and his hands carry learned knowledge into a task his father routinely managed."],
    ["2.A", "The view of the smaller house from the field primarily", "reflects Eli's altered perspective after taking on the repair", "proves the house has physically deteriorated during the year", "shows that the road is closer than the field", "explains why the father sought work elsewhere", "The shift in vantage follows Eli's independent work and makes the familiar home look newly vulnerable, supporting his decision to leave the protective preparation in place."],
    ["3.B", "The windstorm functions as a turning point because it", "converts Eli's passive waiting into unprompted action", "causes the father to telephone after months of silence", "destroys the fence beyond repair", "signals that winter has already begun", "Until the mesh tears free, Eli preserves the fence while claiming to await instructions. The storm requires a choice, and he repairs it before school."],
    ["3.D", "The progression of seasons structures the passage mainly to", "connect changing weather with Eli's changing understanding of preparation and absence", "show that the family moves to a different climate", "document the exact length of the father's job", "establish that the snow fence is useful in summer", "October, winter, March, June, and September measure both the fence's purpose and the father's increasingly uncertain return, bringing Eli to prepare for another unseen winter."],
    ["4.A", "The close third-person narration allows readers to", "recognize gaps between Eli's stated excuses and his less acknowledged motives", "know exactly why the father stops calling", "hear the mother's private thoughts about the marriage", "verify the length of the father's employment contract", "The narration reports Eli telling himself he awaits instructions and then shows behavior suggesting attachment, uncertainty, and learned responsibility without resolving the father's motives."],
    ["4.B", "The phrase “his hands remembered it before his mind did” suggests that", "practical knowledge and identification with his father persist beneath conscious hesitation", "Eli repairs the fence while sleepwalking", "the task is too complex for Eli to understand", "his father has returned secretly to guide him", "The figurative phrasing presents embodied learning: Eli has absorbed the work even before consciously claiming the role or deciding what the fence now means."],
...
And zero at the bone.`,
    ...
    ["5.A", "Describing the snake as a “fellow” chiefly", "combines social familiarity with an unsettling refusal to domesticate the creature", "identifies the animal as a human neighbor", "makes the speaker's final fear seem insincere", "proves the snake recognizes the speaker personally", "The familiar noun places the snake among “nature's people,” yet the speaker never achieves the cordial relationship enjoyed with other creatures."],
    ["5.B", "The image of grass dividing “as with a comb” makes the snake visible through", "the orderly line its motion briefly creates", "the sound of its scales striking metal", "a permanent trail cut into the field", "the speaker's act of parting the grass by hand", "A comb parts strands into a line; the simile shows grass separating and closing, so the unseen body is perceived by its transient effect."],
...
Dare frame thy fearful symmetry?`,
    ...
    ["1.A", "The speaker is characterized chiefly by", "awed questioning in the presence of terrifying creative power", "confidence in a complete explanation of the animal's origin", "indifference toward the tiger's physical beauty", "desire to hunt and destroy the tiger", "The speaker asks rather than answers, repeatedly joining admiration for form and energy with dread about the power capable of making them."],
    ["3.B", "The change from “Could frame” in the opening to “Dare frame” in the ending chiefly", "intensifies the question from capacity to moral or imaginative audacity", "resolves the identity of the creator", "shows that the tiger has become less frightening", "replaces admiration with scientific certainty", "“Could” asks what power was able to create the tiger; “dare” asks what power would risk or presume to do so, deepening rather than resolving the mystery."],
    ["3.D", "The poem's sequence of questions has the cumulative effect of", "expanding inquiry from the tiger's appearance to the creator's means and motives", "narrating the tiger's movement through a forest", "proving that no creator could have made the tiger", "cataloguing the animal's prey", "Questions move from hand and eye to cosmic fire, body, forge, and the Lamb, progressively enlarging the problem of creation without supplying an answer."],
    ["4.A", "The direct address to the tiger primarily makes it", "an immediate object of contemplation rather than a distant example", "the speaker capable of answering the poem's questions", "a harmless pet familiar to the speaker", "the narrator of the middle stanzas", "Apostrophe places the tiger before the speaker in vivid imaginative presence, although the questions ultimately concern the unobserved creator."],
    ["4.B", "The speaker's attitude combines", "admiration for form with dread of the power that formed it", "contempt for the tiger with pity for its weakness", "certainty about divine benevolence with casual humor", "scientific curiosity with emotional neutrality", "“Burning bright” and “symmetry” convey beauty and order, while “fearful,” “dread,” and “terrors” register danger and anxiety about creation."],
    ["4.C", "The lack of declarative answers most strongly suggests that the speaker", "regards the tiger's existence as a mystery that inquiry can sharpen but not settle", "has forgotten the questions immediately after asking them", "expects the tiger to provide a theological explanation", "believes the questions are unimportant", "The poem's formal insistence on unanswered questions makes wonder and uncertainty its mode; repetition strengthens the problem rather than dismissing it."],
    ["5.A", "The phrase “burning bright” primarily associates the tiger with", "radiant energy that is both beautiful and dangerous", "literal injury caused by a forest fire", "gentle warmth offered to other animals", "a light used to guide travelers at night", "Fire imagery gives the tiger brilliance, force, and threat against darkness, fitting the poem's union of aesthetic admiration and terror."],
    ["5.B", "The forge imagery in the fourth stanza presents creation as", "violent, skilled labor shaping formidable material", "effortless growth within a peaceful landscape", "a legal judgment delivered by the stars", "an accidental event without an agent", "Hammer, chain, furnace, anvil, and grasp imagine a maker exerting craft and strength, turning creation into dangerous industrial work."],
    ["6.A", "The question about the maker of “the Lamb” chiefly compares", "gentleness and vulnerability with terrifying strength", "two predators competing in the same forest", "human craftsmanship with animal instinct", "nighttime darkness with daylight", "Invoking the Lamb brings an emblem of innocence and meekness beside the tiger, intensifying the theological problem of one creator encompassing apparent opposites."],
    ["6.B", "The stars' spears and tears juxtapose", "cosmic conflict with grief or awe", "agricultural labor with seasonal rain", "the tiger's prey with its offspring", "human technology with forest growth", "Militarized “spears” suggest struggle or surrender, while tears introduce sorrow or reverence, enlarging the emotional response to creation onto a cosmic scale."],
    ["7.B", "Which claim best captures the poem's treatment of creation?", "A created form may display order and beauty while making its creator's purposes more troubling, not less.", "Beauty proves that creative power is necessarily gentle.", "Destructive strength cannot coexist with formal symmetry.", "The natural world offers simple answers to questions about moral order.", "The tiger's “fearful symmetry” fuses design with terror, and the Lamb comparison makes shared authorship a question rather than a reassuring solution."],
  ]);
...
And I was unaware.`,
    ...
    ["5.A", "The “weakening eye of day” personifies sunset as", "failing vision consistent with age and decline", "a watchful guardian protecting the speaker", "a source of warmth returning to the land", "an artist admiring the tangled vines", "The weakening eye joins diminished light with bodily decline, supporting the century-as-corpse imagery that follows."],
    ["5.B", "The broken-lyre image contributes to the first stanza by suggesting", "a landscape whose capacity for harmony appears damaged", "music that anticipates the thrush's cheerful song", "vines arranged deliberately by a musician", "the speaker's desire to repair an instrument", "Bare stems resemble snapped strings, turning silence and disorder into a visual image that the later living song unexpectedly counters."],
    ["6.A", "The contrast between the thrush's frailty and its “joy illimited” emphasizes", "the disproportion between the singer's weakness and the song's force", "the bird's ignorance of approaching winter", "the speaker's superior physical health", "the certainty that young birds cannot sing", "Aged, gaunt, small, and wind-ruffled describe a weak body, while unlimited joy and flinging its soul convey extraordinary expressive energy."],
    ["6.B", "The “good-night air” and “growing gloom” together associate the song with", "an ending into which hope is voiced rather than a darkness already overcome", "morning sunlight that has begun to disperse clouds", "a celebration attended by nearby households", "the bird's attempt to call a mate", "Night continues to grow; the song occurs within the ending and does not erase it, which makes its possible hope more paradoxical."],
    ["7.B", "Which interpretation best accounts for the poem's ending?", "The speaker becomes open to hope without finding external evidence sufficient to possess it confidently.", "The landscape visibly proves that renewal has already begun.", "The thrush communicates a message whose content the speaker fully understands.", "The speaker concludes that joyful expression is irrational and worthless.", "Nothing terrestrial supplies cause for the song, yet the speaker imagines the possibility of blessed Hope and simultaneously acknowledges remaining unaware."],
  ]);
...
We wear the mask!`,
    ...
    ["1.A", "The collective speaker's repeated use of “we” primarily", "presents concealment as a shared condition rather than a private idiosyncrasy", "identifies every person in the world as equally deceptive", "shows that the speaker is a formal organization", "avoids expressing any emotional pain", "The plural voice binds torn hearts, cries, and performed smiles into a collective experience, while “the world” remains a distinct observing audience."],
    ["3.B", "The direct appeal to Christ in the final stanza marks a shift from", "concealment before the world to disclosure before a divine listener", "private sorrow to public celebration", "singing to complete silence", "a collective voice to a single named speaker", "The group lets the world see the mask but directs its true cries to Christ, creating an audience before whom hidden suffering may be spoken."],
    ["3.D", "The refrain “We wear the mask” functions chiefly to", "return each development to the necessity of sustained performance", "show that the speakers remove the mask between stanzas", "resolve the conflict between pain and public appearance", "identify the physical material from which the mask is made", "After describing concealment, questioning the world, and appealing to Christ, the refrain persists, indicating that recognition has not ended the social performance."],
    ["4.A", "The rhetorical question in the second stanza suggests that the speakers", "defensively reject the world's claim to inspect their suffering", "eagerly invite the world to count every tear", "do not experience the pain described in the first stanza", "expect an explicit answer from Christ", "“Why should” and “Nay” challenge the world's entitlement to knowledge and affirm control over what the observers are permitted to see."],
    ["4.B", "The speakers' attitude toward “the world” is best described as", "guarded and distrustful", "openly dependent and confessional", "affectionate and reassuring", "unaware of being observed", "The world is allowed a constructed smile and dream because fuller knowledge is treated as intrusive, establishing distrust and strategic concealment."],
    ["4.C", "The exclamation point in the final refrain most plausibly intensifies", "the force and cost of continuing the declaration", "a joyful decision to abandon concealment", "a request that the world provide a new mask", "uncertainty about whether the mask exists", "After the stanza exposes tortured souls and a vile road, the emphatic refrain sounds both resolute and painful, not celebratory or doubtful."],
    ["5.A", "The mask that “grins and lies” symbolizes", "a socially legible appearance that conceals suffering", "a religious object used during prayer", "the world's accurate understanding of the speakers", "a cure for emotional pain", "The mask produces the smile outsiders see while hiding cheeks, eyes, tears, and cries, making it the governing symbol of performed well-being."],
    ["5.B", "The phrase “torn and bleeding hearts” chiefly makes the hidden pain", "viscerally physical in contrast to the visible smile", "minor and easily repaired", "abstract and intellectually distant", "a consequence of wearing a literal mask", "Bodily injury language gives concealed anguish urgency and sharpens the contradiction of smiling through it."],
    ["6.A", "The contrast between “smile” and “cries” emphasizes", "the division between outward performance and inward experience", "a progression from sorrow to permanent happiness", "the speakers' inability to make any sound", "the world's preference for singing over speech", "The poem repeatedly places cheerful public expressions beside suffering audible only to Christ, making the split central rather than sequentially resolved."],
    ["6.B", "The juxtaposition of singing with “the clay is vile” suggests", "an uplifting appearance maintained during a degrading, difficult journey", "a festival held on fertile farmland", "the speakers' pleasure in the road's physical conditions", "the world's accurate perception of hardship", "Song continues while the ground is vile and the mile long, pairing performed uplift with ongoing hardship that the world is allowed to misread."],
    ["7.B", "Which claim is best supported by the poem?", "Concealment can operate as a collective strategy even while it exacts emotional costs.", "Public misunderstanding disappears once private suffering is acknowledged to oneself.", "The speakers conceal pain because they are unable to describe it.", "The world responds compassionately whenever suffering becomes visible.", "The speakers articulate pain clearly to Christ and readers, yet deliberately maintain the mask before a distrusted world; torn hearts and tortured souls show the strategy's cost."],
  ]);
...
Yea, beds for all who come.`,
    ...
    ["7.B", "Which interpretation best fits both the hardship and reassurance in the poem?", "The promise of rest gives meaning to struggle without pretending the struggle is brief.", "A worthy destination makes fatigue impossible during the journey.", "Only travelers who ask no questions are admitted to the inn.", "Uncertainty is resolved by turning away from the uphill road.", "The guide confirms a full-day climb and weariness while assuring refuge. The poem comforts by placing hardship within a meaningful destination, not by denying it."],
  ]);
...
MAYOR: Put the item last. We need time to consider it.`,
    ...
    ["2.A", "The two visible clocks contribute most directly to", "a setting where competing versions of official reality are physically present", "a suspenseful threat that the building may collapse", "evidence that the meeting occurs in different time zones", "a private domestic conflict unrelated to government", "The contradictory clocks materialize the council's problem: official signals have been adjusted around one another until no shared time is straightforward."],
    ["3.B", "The proposal for an accurate mechanism is the scene's turning point because it", "changes inaccurate time from a comic inconvenience into a decision about altering tradition", "reveals that the courthouse clock has always been correct", "causes Mr. Pike to leave the chamber", "resolves which clock governs the meeting", "Before the proposal, characters navigate the absurd system; a genuine repair forces them to confront whether they want the familiar error removed."],
    ["3.D", "The final decision to put the item last chiefly", "returns the scene's time problem in the form of procrastination", "shows that the Mayor has calculated the meeting length", "guarantees the repair will pass unanimously", "introduces a new conflict about funding", "“Need time” turns a clock repair into delayed action, providing comic closure by reproducing the very disorder the agenda item could solve."],
    ["4.B", "The stage direction in which all three look uneasily at the window indicates", "shared attachment to the familiar clock despite their complaints", "fear that the clock will fall into the chamber", "certainty that residents demand immediate repair", "confusion about where the courthouse is located", "Their silent reaction appears only when accuracy is proposed, revealing an emotional investment that their earlier practical criticism did not acknowledge."],
    ["7.A", "A defensible interpretation of the clock is that it represents", "an institution's tendency to accommodate a known flaw until the flaw becomes tradition", "technological progress welcomed without reservation", "the unavoidable difference between public and private morality", "a natural cycle beyond human control", "The town has built other procedures around the error, labeled it historical, and becomes uneasy when an invisible correction is offered."],
    ["7.B", "Which detail best supports the scene's satire of bureaucracy?", "Public Works makes one clock slow to compensate for citizens relying on a fast clock.", "Mr. Pike arrives while breathing hard.", "The council chamber has a high window.", "The proposed mechanism will leave the clock face unchanged.", "Rather than correct the first problem, one department creates an opposing error, an exaggerated example of procedure multiplying around an accepted defect."],
    ["7.C", "The scene's comic effect depends most on the tension between", "the characters' language of precision and their acceptance of systematic inaccuracy", "the Mayor's youth and Mr. Pike's age", "the courthouse exterior and council chamber furniture", "the cost of repairs and the Clerk's salary", "Claims about official time, punctuality, and accurate mechanisms repeatedly collide with clocks intentionally fast or slow and with postponement of the only true repair."],
  ]);
...
LUIS: For now.`,
    ...
    ["2.A", "The closed kitchen setting intensifies the conflict because it", "surrounds the characters with the labor and objects to which they assign different meanings", "allows restaurant customers to interrupt their private disagreement", "proves the building can easily become a pharmacy", "places the family far from any reminder of debt", "Containers, pan, tiles, and refrigerator make the inheritance tangible while the private after-hours space lets practical work and family argument merge."],
    ["3.B", "The refrigerator's sudden silence functions as a turning point by", "temporarily replacing verbal argument with a shared fear about the failing business", "confirming that every appliance has stopped working", "causing Ana to withdraw the bank offer", "revealing that Luis has deliberately unplugged it", "Both characters turn together, and the mechanical uncertainty briefly embodies the larger question of how long the restaurant can continue."],
    ["3.D", "The final exchange “Still cold” / “For now” chiefly", "condenses the conflict into the difference between present function and future viability", "settles the sale in Ana's favor", "shows that Luis no longer values the restaurant", "introduces evidence that the food has spoiled", "Ana verifies the immediate condition; Luis answers with temporal uncertainty. The lines preserve both practical fact and emotional reluctance without resolving the decision."],
    ["4.B", "Luis's jokes about Fridays and roads primarily", "deflect direct engagement while revealing his resistance", "demonstrate that he does not understand Ana's statements", "show that he wants Mateo to remain in Denver", "provide factual information about banking and transportation", "The witty replies avoid the underlying debt and abandonment concerns, but their direction—against the offer and toward possible return—makes his resistance clear."],
    ["7.A", "The repeatedly scrubbed pan can reasonably be interpreted as", "Luis's attempt to preserve control through familiar labor", "proof that Ana has neglected all kitchen work", "a sign that the restaurant has many customers waiting", "evidence that the bank requires an equipment inspection", "Luis works beyond practical necessity while resisting the sale, turning an ordinary task into a manageable substitute for the decision he cannot cleanly resolve."],
    ["7.B", "Which detail most strongly supports the claim that Ana's practicality does not erase her attachment?", "She says she knows the refrigerator's habits because she grew up in the restaurant.", "She repeats that the offer will pay the debt.", "She notes that Mateo has lived away for eleven years.", "She labels containers after the restaurant closes.", "The line “I know. I grew up here” asserts intimate knowledge and history even as Ana checks the failing equipment, complicating any view of her as merely financial."],
    ["7.C", "The scene presents the sale chiefly as a conflict between", "preserving memory in a place and preserving family agency beyond that place", "two equally profitable commercial uses for the building", "Ana and Mateo over who should inherit the restaurant", "cleanliness standards and customer preferences", "Luis treats the building as the family's material continuity; Ana argues that making it carry the family can trap its members, so both positions concern preservation in different forms."],
  ]);

  addSet({
    id: "ld-orbit", unit: "LD", type: "longer-drama", era: "20th-century style",
...
RUTH: Only because you keep answering both with your name.`,
    ...
    ["2.A", "The 1936 observatory setting is significant chiefly because it", "places the credit dispute within institutional barriers Vale invokes against Ruth", "guarantees that photographic plates cannot produce reliable evidence", "shows that editors did not publish scientific tables in that era", "makes the possible companion star irrelevant to the conflict", "Vale explicitly cites the board's refusal to appoint a woman, turning the workplace and period into forces shaping whose name can carry scientific authority."],
    ["3.B", "Ruth's line “Put my name beside the error” is the turning point because it", "answers Vale's caution by accepting the risk that comes with recognition", "concedes that her observation is merely a damaged plate", "asks Vale to remove his own name from the paper", "changes the discussion from authorship to salary", "The line defeats the suggestion that she wants credit only if correct; she claims both accountability and recognition, clarifying the ethical basis of her demand."],
    ["3.D", "Placing the plate over the signed journal visually", "sets Ruth's observational evidence across Vale's printed authority", "destroys the only record of the possible star", "shows that the journal page and plate are identical", "signals that Ruth has accepted the editor's decision", "The dark line crossing the title creates a stage image of data interrupting an authorship system dominated by Vale's repeated name."],
    ...
    ["7.A", "The possible companion star most plausibly parallels", "Ruth's labor as a consequential presence the institution initially labels a flaw or leaves unnamed", "Vale's position as an already recognized source of light", "the editor's successful removal of unnecessary data", "the board's certainty about the observatory's future", "Ruth identifies a faint signal that Vale dismisses before looking, while her own substantial work is compressed into “staff”; both require recognition of something obscured."],
    ["7.B", "Which evidence best supports an interpretation that Vale sees himself as protecting access rather than simply taking credit?", "He argues that his recognized name can secure publication of the next paper.", "He caps his pen after Ruth mentions the first page.", "He calls the August observation a flaw.", "He tells Ruth to sit down.", "The publication argument presents his authority as instrumental for future work, complicating—but not erasing—the self-serving effect of excluding Ruth."],
    ["7.C", "The final exchange reveals that the deepest conflict concerns", "whether existing power should be used to reproduce or challenge unequal recognition", "whether the companion star exists in the photographed region", "which person will physically occupy Vale's chair", "whether the editor shortened other parts of the article", "Vale treats Ruth's credit as inseparable from a barred appointment and therefore impossible; Ruth argues that his repeated acceptance of that logic helps sustain it."],
  ]);
