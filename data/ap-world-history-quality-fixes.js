// AP World History: Modern — reviewed quality repairs.
...
(() => {
  ...
  if(!Array.isArray(bank)) throw new Error("AP World bank must load before quality fixes");
  const replacements={
  ...
  for(const [id,distractors] of Object.entries(replacements)){
    ...
    if(!q) throw new Error(`Missing AP World quality-fix item ${id}`);
