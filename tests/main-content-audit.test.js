...
function tokenSet(s=''){return new Set(normalize(s).split(/\s+/).filter(Boolean));}
...
const giveaway=/\b(obviously|clearly|ridiculous|nonsense|unrelated|randomly|magically|because it just does|none of the above|all of the above)\b/i;
...
for (const [name, bank] of Object.entries(banks)) {
  test(`AUDIT ${name}`, () => {
    ...
    const weak=[]; const shortDistr=[]; const absolutes=[]; const outlierCorrect=[]; const nearDup=[];
    ...
    console.log(JSON.stringify({
      ...
      flagged:{giveaway:weak, oneWordVsLong:shortDistr, stackedAbsolute:absolutes, correctLengthOutlier:outlierCorrect, nearDuplicateDistractors:[...new Set(nearDup)]}
    }));
  });
}
