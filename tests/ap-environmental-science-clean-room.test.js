...
test('APES clean-room Practice 7 items prescribe a concrete environmental intervention', () => {
  ...
  const action = /reduce|remove|restore|protect|use|produce|install|expand|improve|limit|retire|replace|stabilize|contain|apply|adjust|prevent|phase out|plan|site|adopt|develop|require|capture|control|manage|clean|drain|dry|enforce|increase reflective|match fertilizer/i;
...
test('APES clean-room source practices depend on the declared source type', () => {
  ...
  for (const q of source) {
    ...
    if (q.stimulus.type === 'quantitative') {
      ...
      assert.match(q.q, /quantitative source|data|value|calculate|percent|change|relationship|table|rate|difference|efficien|shown|times greater|times (?:as )?(?:large|high|much)|ratio/i, `${q.id}: quantitative task does not require quantitative evidence`);
